...
let editTaskText = (event) => {
  if(event.detail === DOUBLE_CLICK){
    ...
    console.log(Array.from(event.target.parentNode.parentNode.childNodes))
    Array.from(event.target.parentNode.parentNode.childNodes).forEach((elem)=>{
      if(elem.id!==event.target.parentNode.parentNode.id){
        elem.firstElement.previousSibling.hidden = true;
        elem.lastElement.previousSibling.hidden = false;
      }
    })
  }
}
