...
const showTaskTab = (parentCurrentTarget,currentName) => {
  let copyTasks = [...tasks]
...
const showTaskCompleted = () => {
  let copyTasks = [...tasks].filter((task)=>task.isChecked)
  return copyTasks
...
const typeFilter = (event)=> {
  ...
  if(parentCurrentTarget.name === 'check-active'){
    showTaskActive(copyTasks,parentCurrentTarget)
    addActiveStyle(parentCurrentTarget)
  } 
  if(parentCurrentTarget.name === 'check-completed') {
    showTaskCompleted(copyTasks,parentCurrentTarget)
    addActiveStyle(parentCurrentTarget)
  }
}
