...
let editTaskText = (event) => {
  if(event.detail === DOUBLE_CLICK){
    Array.from(event.target.parentNode.parentNode.children).forEach((elem)=>{
      console.log(elem)
      if(Number(elem.id)!==Number(event.target.parentNode.parentNode.id)){
        console.log(1)
        elem.firstElement.previousElementSibling.hidden = true;
        elem.lastElement.previousElementSibling.hidden = false;
      }
    })
