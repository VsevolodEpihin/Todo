(()=>{
  ...
  const markTask = (currentTask) => {
    console.log(currentTask.parentNode.id)
    tasks.forEach((task) => {
      console.log(task.id,Number(currentTask.parentNode.id))
      if (Number(currentTask.parentNode.id) === task.id) {
        task.isChecked = parent.checked;
      }
    });
    changeGlobalCheckbox()
    renderTask();
  ...
  const selectActionTask = (event) => {
    ...
    if (event.target.type === 'checkbox') markTask(event.target);
