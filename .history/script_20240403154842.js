(()=>{
  ...
  const editTaskText = (event) => {
    if (event.detail === DOUBLE_CLICK) {
      ...
      console.log(event.target.parentNode.lastElementChild.previousElementSibling.textContent)

      event.target.parentNode.firstElementChild.nextElementSibling.value = event.target.parentNode.lastElementChild.previousElementSibling.value;
    }
  };
