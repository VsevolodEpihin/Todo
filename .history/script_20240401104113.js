...
const renderTaskCompleted = (copyTask,parentCurrentTarget) => {
  ...
  parentCurrentTarget.firstElementChild.textContent = copyTask.length;
  renderTask(copyTask)
}
...
let counterTasks = () => {
  tasks.forEach((task) => {
    if(task.isChecked){
      optionButtons.lastElementChild.firstElementChild.textContent = Number(optionButtons.lastElementChild.firstElementChild.textContent)+1 
    }
    if(!task.isChecked){
      optionButtons.firstElementChild.nextElementSibling.firstElementChild.textContent = Number(optionButtons.firstElementChild.nextElementSibling.firstElementChild.textContent)+1 
    }
  })
  optionButtons.firstElementChild.firstElementChild.textContent = Number(optionButtons.firstElementChild.firstElementChild.textContent)+1
}
...
const renderTaskAll = (parentCurrentTarget) => {
  parentCurrentTarget.firstElementChild.textContent = tasks.length;
  renderTask()
} 
...
const typeFilter = (event)=> {
  ...
  let copyTasks = [...tasks]
  if(parentCurrentTarget.name === 'check-all') renderTaskAll(parentCurrentTarget)
