...
let writeChanges = (event) => {
  ...
  if(event.code === 'Escape'){
    console.log(currentText.nextElementSibling.textContent,currentText.nextElementSibling.data)
    currentText.nextElementSibling.textContent = currentText.nextElementSibling.data
    currentText.nextElementSibling.hidden = false;
    currentText.hidden = true;
    console.log(2)
  }
  if(event.type === 'blur' && event.code!=='Escape'){
    console.log(currentText, currentText.value)
    currentText.nextElementSibling.data = currentText.nextElementSibling.textContent
    console.log(currentText.nextElementSibling.data)
    currentText.nextElementSibling.textContent = currentText.value
    currentText.nextElementSibling.hidden = false;
    currentText.hidden = true;
    console.log(3)
