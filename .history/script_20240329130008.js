...
let editTaskText = (event) => {
  let repeatChanges = false;
  console.log(Array.from(event.target.parentNode.parentNode.children))
  Array.from(event.target.parentNode.children).forEach((elem)=>{
    console.log(elem.hasAttribute('hidden'))
    if(elem.hasAttribute('hidden')){
      repeatChanges = true;
    }
  })
  if(event.detail === DOUBLE_CLICK && !repeatChanges){
