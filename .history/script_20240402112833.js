...
const pagination = (currentListTasks) => {
  
  let pages = Math.ceil(currentListTasks.length / rows);
  let btnPaginate = ''
  for(let i = 1; i <= pages; i++){
    console.log(i)
    btnPaginate+=`<button id="paginate-${i}">${i}<button/>`
  }
  console.log(btnPaginate)
...
const renderTask = () => {
   ...
   let paginate = pagination(currentListTasks)
...
paginationButtons.addEventListener('click',paginateFilter)