...
let counterTasks = () => {
  tasks.forEach((task) => {
    if(task.isChecked){
      Number(optionButtons.lastElementChild.firstElementChild.textContent) +=1 
    }
    if(!task.isChecked){
      Number(optionButtons.firstElementChild.nextElementSibling.firstElementChild.textContent) +=1 
    }
  })
  Number(optionButtons.firstElementChild.firstElementChild.textContent) +=1
}
